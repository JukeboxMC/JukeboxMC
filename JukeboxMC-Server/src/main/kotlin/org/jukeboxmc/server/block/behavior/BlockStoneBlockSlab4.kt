...
class BlockStoneBlockSlab4(identifier: Identifier, blockStates: NbtMap?) : JukeboxBlock(identifier, blockStates),
    StoneBlockSlab4 {
    ...
    override fun placeBlock(
        ...
        if (blockFace === BlockFace.DOWN) {
            if (targetBlock is BlockStoneBlockSlab4 && targetBlock.getVerticalHalf() == VerticalHalf.TOP && targetBlock.getStoneSlabType4() == this.getStoneSlabType4()) {
                world.setBlock(
                    blockPosition,
                    Block.create<BlockDoubleStoneBlockSlab4>(BlockType.DOUBLE_STONE_BLOCK_SLAB4)
                        .setStoneSlabType4(this.getStoneSlabType4())
                )
                return true
