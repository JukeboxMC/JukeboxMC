package org.jukeboxmc.server.inventory

import org.cloudburstmc.protocol.bedrock.data.inventory.ContainerType
import org.jukeboxmc.api.inventory.BlastFurnaceInventory
import org.jukeboxmc.api.inventory.InventoryHolder
import org.jukeboxmc.api.inventory.InventoryType
import org.jukeboxmc.server.blockentity.JukeboxBlockEntityBlastFurnace

class JukeboxBlastFurnaceInventory(inventoryHolder: InventoryHolder) : ContainerInventory(inventoryHolder, 3), BlastFurnaceInventory {

    override fun getInventoryHolder(): JukeboxBlockEntityBlastFurnace {
        return super.getInventoryHolder() as JukeboxBlockEntityBlastFurnace
    }

    override fun getType(): InventoryType {
        return InventoryType.BLAST_FURNACE
    }

    override fun getContainerType(): ContainerType {
        return ContainerType.BLAST_FURNACE
    }

}