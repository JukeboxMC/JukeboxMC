...
class ItemPalette {
    ...
    companion object {
        ...
        init {
            val stream = ItemPalette::class.java.classLoader.getResourceAsStream("item_palette.json") ?: throw RuntimeException("The item palette was not found")
            val gson = Gson()
            ...
            stream.reader().use {
                ...
                for (entry in parseItem) {
                    this.itemDefinitions.add(SimpleItemDefinition(entry["name"] as String, (entry["id"] as Double).toInt(), false))
                }
            }
        }
