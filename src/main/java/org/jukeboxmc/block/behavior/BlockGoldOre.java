...
public class BlockGoldOre extends Block {
    ...
    public BlockGoldOre( Identifier identifier ) {
        super( identifier );
    }

    public BlockGoldOre( Identifier identifier, NbtMap blockStates ) {
    ...
    public List<Item> getDrops( Item item ) {
        if ( this.isCorrectToolType( item ) && this.isCorrectTierType( item ) ) {
            ...
            return Collections.singletonList( Item.create( ItemType.RAW_GOLD ).setAmount( amount ) );
        }
