...
public class BinaryStream {
    ...
    public void writeItem(Item item) {
        ...
        try (LittleEndianByteBufOutputStream stream = new LittleEndianByteBufOutputStream(userData)) {
            ...
            if (nbtData != null && !nbtData.isEmpty()) {
                ...
                NBTOutputStream networkWriter = NbtUtils.createNetworkWriter(stream);
                networkWriter.writeTag(nbtData);
            } else {
                userData.writeShortLE(0);
            }

            userData2.writeSignedVarInt(0);
            userData2.writeSignedVarInt(0);

            if (runtimeId == 355) {
                userData2.writeSignedVarLong(0);
            }
    ...
    public Item readItem() {
        ...
        ByteBuf userData = this.readBytes(this.readUnsignedVarInt());
        ...
        int countPlacedOn = userData2.readSignedVarInt();
        for (int i = 0; i < countPlacedOn; i++) {
            userData2.readString();
        }

        int countCanBreak = userData2.readSignedVarInt();
