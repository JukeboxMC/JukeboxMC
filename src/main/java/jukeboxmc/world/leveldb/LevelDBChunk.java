package jukeboxmc.world.leveldb;
        ...
        switch ( subchunkVersion ) {
            ...
            case 1:
                for ( int layer = 0; layer < storages; layer++ ) {
                    ...
                    Map<Short, Integer> chunkPaletteMap = new HashMap<>();
                    ...
                    while ( i < needed ) {
                        try {
                            ...
                            if ( states != null ) {
                                chunkPaletteMap.put( i++, BlockPalette.getRuntimeId( identifier, states ) );
                            } else {
                                chunkPaletteMap.put( i++, BlockPalette.getRuntimeId( identifier, NbtMap.EMPTY ) );
                            }
                        } catch ( IOException e ) {
                            e.printStackTrace();
                            break;
                    ...
                    for ( short index = 0; index < indexes.length; index++ ) {
                        chunk.blocks[layer * 4096 + index] = chunkPaletteMap.get( indexes[index] );
                    }
                }
                buffer.release();
                break;
